...
//                    camera.startPreview();
    ...
    @Override
    void setOrientation(int displayOrientation, int sensorOrientation) {
        mDisplayOrientation = displayOrientation;
        mSensorOrientation = sensorOrientation;
        if (isCameraOpened()) {
            int cameraDisplayRotation = calcCameraDisplayRotation(displayOrientation);
            int cameraPictureRotation = calcCameraPictureRotation(sensorOrientation);
//            mCameraParameters.setRotation(cameraPictureRotation);
            mCamera.setParameters(mCameraParameters);
            final boolean needsToStopPreview = mShowingPreview && Build.VERSION.SDK_INT < 14;
            if (needsToStopPreview) {
                mCamera.stopPreview();
    ...
    private void openCamera() {
        ...
        adjustCameraParameters();
        mCamera.setDisplayOrientation(calcCameraDisplayRotation(mDisplayOrientation));
...
//            mCameraParameters.setRotation(calcCameraPictureRotation(mSensorOrientation));
    ...
    private int calcCameraDisplayRotation(int rotation) {
    ...
    private int calcCameraPictureRotation(int orientation) {
        int rotation;
        if (mCameraInfo.facing == Camera.CameraInfo.CAMERA_FACING_FRONT) {
